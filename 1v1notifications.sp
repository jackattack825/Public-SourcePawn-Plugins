...
{
	g_enableCVAR = CreateConVar("sm_showupdates", "1", "Controls whether menus are shown or not 1 is on and 0 is off");
	RegAdminCmd("sm_updates", CMD_showUpdates, ADMFLAG_GENERIC);
	RegAdminCmd("sm_commands", CMD_showCommands, ADMFLAG_GENERIC);
	RegAdminCmd("sm_adminlist", CMD_showAdmins, ADMFLAG_GENERIC);
}

public void OnClientPostAdminCheck(int client)
{
	CreateTimer(10.0, timerCallBack, client);
	CreateTimer(22.0, commandsCallBack, client);
	CreateTimer(34.0, adminsCallBack, client);
	
}

public Action timerCallBack(Handle timer, int client)
{
	if(x==0 && GetConVarBool(g_enableCVAR))
	{
		BuildMenu();
		DisplayMenu(g_Menu, client, 10);
	}
	x++;
}

public Action commandsCallBack(Handle timer, int client)
{
	if(y==0 && GetConVarBool(g_enableCVAR))
	{
		buildComms();
		DisplayMenu(g_Comms, client, 10);
	}
	y++;
}

public Action adminsCallBack(Handle timer, int client)
{
	if(z==0 && GetConVarBool(g_enableCVAR))
	{
		buildAdmins();
		DisplayMenu(g_Admins, client, 10);
	}
	z++;
}
...
public void buildComms()
{
		...
		AddMenuItem(g_Comms, "", "!challenge (challenege a player to a fight next round)");
